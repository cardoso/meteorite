...
class NodeRandomGenerator extends AbstractRandomGenerator {
  fraction(): number {
  ...
  hexString(digits: number): string {
    ...
    // Dynamically require crypto to prevent bundler errors on the client
...
export type ExtendedRandomGenerator = RandomGeneratorType & {
  ...
  alea?: any; // To fulfill legacy testing hooks if the primary generator is Alea
};

function createRandom(baseGenerator: AbstractRandomGenerator): ExtendedRandomGenerator {
  const extended = baseGenerator as ExtendedRandomGenerator;

  extended.createWithSeeds = (...seeds: any[]) => {
    if (seeds.length === 0) {
  ...
  extended.insecure = createAleaGeneratorWithGeneratedSeed();
